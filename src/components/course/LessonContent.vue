<template>
  <div class="lesson-content">
    ...
    <template v-for="(block, i) in lesson.content" :key="i">
      <p v-if="block.type === 'text'" class="text-block">{{ block.value || block.text }}</p>
      <h3 v-else-if="block.type === 'heading'" class="heading-block">{{ block.value || block.text }}</h3>
      <CodeBlock v-else-if="block.type === 'code'" :code="block.value || block.code" :language="block.language || 'java'" />
