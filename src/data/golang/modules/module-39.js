  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'fanIn принимает variadic ...<-chan T — можно передать любое количество входных каналов',
          'sync.WaitGroup: Add(len(channels)), каждый форвардер вызывает Done(). Когда все Done — WaitGroup разблокируется',
          'go func() { wg.Wait(); close(out) }() — горутина-наблюдатель закрывает out только когда все форвардеры завершены',
          'Двойной select внутри форвардера: внешний ждёт данные или done, внутренний — отправляет или завершается по done',
          'fanIn используется в паттерне fan-out + fan-in: несколько воркеров → fanIn → единый поток результатов'
        ]},
        ...
        { type: 'tip', value: 'fanIn — это стандартный инструмент мультиплексирования. Используйте его всегда когда нужно объединить результаты нескольких горутин. Не забывайте передавать done-канал — без него при отмене форвардеры могут зависнуть.' }
      ]
    },
    {
      id: 6,
      ...
      content: [
        ...
        { type: 'list', items: [
          'Heartbeat канал обычно буферизованный (size=1): sendHeartbeat с default не блокируется если никто не читает прямо сейчас',
          'Монитор heartbeat использует time.After для обнаружения "молчания": если сигнала нет больше N секунд — воркер завис',
          'Два типа heartbeat: по событию (при каждой обработке задачи) и по таймеру (каждые N секунд даже без задач)',
          'Паттерн используется в реальных системах: watchdog-процессы, мониторинг микросервисов, healthcheck endpoints',
          'При получении heartbeat через time.After(1s): если воркер занят долгой операцией, он не отправит heartbeat — нужно учитывать ожидаемое время выполнения'
        ]},
        ...
        { type: 'tip', value: 'Heartbeat с default (неблокирующий) — стандартный подход: if monitor busy, skip this beat. Это позволяет монитору работать независимо от воркера. Если нужна строгая гарантия доставки — используйте буферизованный канал size=1.' }
      ]
    },
    {
      id: 7,
