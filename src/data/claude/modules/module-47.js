  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'Supervisor оценивает по числовой шкале (0-10) — это удобно для порогового решения: approved = score >= min_score',
          'Итерация с feedback: Worker получает конкретные замечания и исправляет — качество улучшается за 2-3 попытки',
          'max_attempts ограничивает количество попыток — предотвращает бесконечный цикл при неразрешимых требованиях',
          'Supervisor лучше использовать с более мощной моделью чем Worker — критика требует понимания, а не только генерации',
          'ReviewResult — dataclass для структурированного ответа Supervisor, легко расширить дополнительными полями'
        ]},
        ...
        { type: 'tip', value: 'Паттерн Supervisor хорошо работает для задач с чёткими критериями качества: написание документации, генерация тестов, code review. Для творческих задач без чётких критериев — Supervisor может давать противоречивые оценки.' }
      ]
    },
    {
      id: 4,
      ...
      content: [
        ...
        { type: 'list', items: [
          'Дебаты в несколько раундов: второй раунд агенты читают аргументы друг друга — это имитирует реальную дискуссию',
          'Разные perspectives дают разные углы зрения: "оптимист" + "скептик" + "прагматик" охватывают большинство аспектов',
          'Судья (Judge) использует более мощную модель чем участники дебатов — он должен понять все аргументы',
          'Дебаты полезны для архитектурных решений, выбора технологий, оценки рисков — там где нет одного правильного ответа',
          'Результат дебатов лучше одного ответа: агент-судья синтезирует лучшее из нескольких точек зрения'
        ]},
        ...
        { type: 'tip', value: 'Дебаты стоят дорого — каждый агент делает API-вызов за каждый раунд. Используйте Claude Haiku для участников дебатов и Claude Opus только для судьи. Это даёт хорошее качество при разумной стоимости.' }
      ]
    },
    {
      id: 5,
      ...
      content: [
        ...
        { type: 'list', items: [
          'Coder -> Reviewer -> Coder (fix) — итеративный цикл улучшения кода на основе замечаний ревьюера',
          'Reviewer возвращает JSON с approved, issues, suggestions — структурированный ответ проще обрабатывать программно',
          'Tester получает финальный код после ревью — тесты пишутся для одобренной версии, а не черновика',
          'run_code запускает код + тесты в одном файле — простой способ проверить что реализация проходит тесты',
          'Пайплайн Code -> Review -> Test -> Run — полный TDD цикл автоматизированный через агентов'
        ]},
        ...
        { type: 'tip', value: 'Используйте разные модели для разных агентов по стоимости/качеству: Coder — Claude Opus (нужно качество), Reviewer — Claude Opus (нужна строгость), Tester — Claude Haiku (шаблонный код). Это снизит стоимость на 40-60%.' }
      ]
    },
    {
      id: 6,
