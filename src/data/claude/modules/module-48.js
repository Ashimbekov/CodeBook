  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'Тестируйте на одном и том же golden dataset — иначе различия в результатах могут быть из-за данных, а не промпта',
          'judge_fn принимает question, expected, actual и возвращает число — это позволяет менять метрику без изменения фреймворка',
          'llm_judge — самый простой судья через Claude Haiku, но можно использовать любую функцию: regex, similarity и т.д.',
          'improvement_percent = (avg_b - avg_a) / avg_a * 100 — относительное улучшение. Важно: даже +5% статистически значимо на 30+ тестах',
          'Запускайте A/B тест несколько раз и усредняйте — из-за недетерминированности модели один запуск может быть шумным'
        ]},
        ...
        { type: 'tip', value: 'Сохраняйте результаты A/B тестов в файл с timestamp — это позволяет строить историю улучшений промпта. Через месяц вы увидите как качество менялось с каждой итерацией.' }
      ]
    },
    {
      id: 4,
      ...
      content: [
        ...
        { type: 'list', items: [
          'prompt_hash: hash(system_prompt) — позволяет отследить с каким промптом был запущен тест, без хранения полного текста',
          'Структурные проверки (contains, not_contains, length_max) — детерминированы и дёшевы. Запускайте их на каждом PR',
          'regressions = baseline_passed - current_passed — множество тестов что раньше проходили, теперь нет. Это блокеры деплоя',
          'improvements = current_passed - baseline_passed — случаи которые стали лучше. Иногда "неожиданные" улучшения',
          'Запускайте регрессионные тесты в CI/CD при изменении системного промпта или обновлении модели'
        ]},
        ...
        { type: 'tip', value: 'Автоматизируйте в CI: если regression_detected = True — блокируй деплой. Если pass_rate упал более чем на 5% — тоже блокируй. Это защитит production от случайных ухудшений.' }
      ]
    },
    {
      id: 5,
