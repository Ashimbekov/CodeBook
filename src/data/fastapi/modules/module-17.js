  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Тестирование структуры ответа об ошибке' },
        { type: 'code', language: 'python', value: 'def test_validation_error_structure():\n    """Проверяем формат ошибки валидации"""\n    r = client.post("/items/", json={"price": -1})  # нет name\n    assert r.status_code == 422\n    body = r.json()\n    # FastAPI возвращает {"detail": [{"loc": [...], "msg": "...", "type": "..."}]}\n    assert "detail" in body\n    errors = body["detail"]\n    assert isinstance(errors, list)\n    assert len(errors) > 0\n    error = errors[0]\n    assert "loc" in error   # местоположение ошибки\n    assert "msg" in error   # сообщение\n    assert "type" in error  # тип ошибки\n\ndef test_404_error_message():\n    """404 должен возвращать понятное сообщение"""\n    r = client.get("/items/99999")\n    assert r.status_code == 404\n    body = r.json()\n    assert "detail" in body\n    # Сообщение должно быть на русском\n    assert "не найден" in body["detail"].lower() or "not found" in body["detail"].lower()\n\ndef test_method_not_allowed():\n    """405 при неверном HTTP методе"""\n    r = client.patch("/items/")  # PATCH не поддерживается для списка\n    assert r.status_code == 405' },
        { type: 'note', value: 'Граничные случаи для проверки: ID = 0 или отрицательный, очень длинные строки (255+ символов), специальные символы в строках (кавычки, слэши), числа-строки, null вместо объекта, пустой массив вместо объекта.' },
        { type: 'tip', value: 'TestClient из FastAPI использует requests под капотом. Все запросы синхронные, даже если эндпоинты асинхронные. Для настоящего async тестирования используй pytest-asyncio с httpx.AsyncClient.' }
      ]
    },
    {
      id: 6,
