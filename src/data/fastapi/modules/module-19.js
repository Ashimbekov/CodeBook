  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Линтинг и форматирование в CI' },
        { type: 'code', language: 'python', value: '# Добавь шаг lint перед тестами в GitHub Actions:\n# - name: Lint\n#   run: |\n#     pip install ruff mypy\n#     ruff check .          # линтер (заменяет flake8, pylint)\n#     mypy .                # проверка типов\n\n# pyproject.toml — конфигурация инструментов:\n# [tool.ruff]\n# line-length = 88\n# select = ["E", "F", "I"]  # pycodestyle, pyflakes, isort\n#\n# [tool.mypy]\n# python_version = "3.11"\n# strict = true\n# ignore_missing_imports = true\n\n# Makefile для удобства:\n# lint:\n#     ruff check . && mypy .\n# test:\n#     pytest --cov=. --cov-report=term-missing\n# ci: lint test' },
        { type: 'note', value: 'В GitHub Actions используй кэширование зависимостей чтобы ускорить выполнение: actions/cache@v3 для директории ~/.cache/pip. Это сокращает время CI с 3-4 минут до 30-60 секунд при повторных запусках.' },
        { type: 'tip', value: 'Секреты храни ТОЛЬКО в GitHub Secrets (Settings -> Secrets and variables -> Actions). Никогда не коммить .env файлы с реальными ключами — добавь .env в .gitignore и создай .env.example с примерами значений.' }
      ]
    },
    {
      id: 7,
