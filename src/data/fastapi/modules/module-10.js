export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'POST /auth/register с body {"email": "user@mail.com", "password": "Pass123!"} → 201 Created, body: {"access_token": "eyJ...", "refresh_token": "abc123...", "token_type": "bearer"}\nPOST /auth/login (form-data: username=user@mail.com, password=Pass123!) → 200 OK, body: {"access_token": "eyJ...", "refresh_token": "xyz..."}\nGET /auth/me с заголовком Authorization: Bearer eyJ... → 200 OK, body: {"id": 1, "email": "user@mail.com"}\nPOST /auth/refresh с body {"refresh_token": "abc..."} → 200 OK, body: {"access_token": "новый_токен", "refresh_token": "новый_refresh"}\nPOST /auth/logout → 200 OK, body: {"message": "Выход выполнен"}',
    ...
    },
    {
      ...
      expectedOutput: 'POST /posts (авторизованный USER) → 201 Created, body: {"id": 1, "title": "...", "user_id": 1}\nPUT /posts/1 (автор поста) → 200 OK, body: обновлённый пост\nPUT /posts/1 (другой USER, не автор) → 403 Forbidden, body: {"detail": "Нет прав на редактирование"}\nPUT /posts/1 (ADMIN) → 200 OK (ADMIN может всё)\nGET /admin/posts без токена → 401 Unauthorized\nGET /admin/posts с ADMIN токеном → 200 OK, все посты включая черновики\nPATCH /admin/users/2/role (USER токен) → 403 Forbidden',
