  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Отправка JSON и типизированных сообщений' },
        { type: 'code', language: 'python', value: '# Вместо send_text можно использовать send_json — он автоматически сериализует dict\n@app.websocket("/ws/room/{room}/{user}")\nasync def typed_room(websocket: WebSocket, room: str, user: str):\n    await websocket.accept()\n    # Отправить JSON сообщение\n    await websocket.send_json({"type": "welcome", "room": room, "user": user})\n    try:\n        while True:\n            # Принять JSON от клиента\n            data = await websocket.receive_json()\n            message = {\n                "type": "message",\n                "user": user,\n                "text": data.get("text", ""),\n                "room": room\n            }\n            await manager.broadcast_room(room, json.dumps(message))\n    except WebSocketDisconnect:\n        manager.leave(room, websocket)' },
        { type: 'heading', value: 'Список активных комнат' },
        { type: 'code', language: 'python', value: '# REST эндпоинт для получения информации о комнатах\n@app.get("/rooms")\ndef get_rooms():\n    return {\n        room: {"count": manager.get_room_count(room)}\n        for room in manager.rooms\n    }\n\n# Пример ответа:\n# {\n#   "general": {"count": 5},\n#   "tech": {"count": 3},\n#   "random": {"count": 12}\n# }' },
        { type: 'tip', value: 'При использовании нескольких воркеров (uvicorn --workers N) каждый воркер имеет свой RoomManager. Для синхронизации между воркерами используют Redis Pub/Sub или Redis Streams. Для одного воркера (что достаточно для большинства задач) простой словарь в памяти работает отлично.' }
      ]
    },
    {
      id: 6,
