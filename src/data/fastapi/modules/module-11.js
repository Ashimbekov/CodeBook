export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'GET /health → 200 OK, body: {"status": "ok", "version": "1.0"}, headers: X-Request-ID: a1b2c3d4, X-Content-Type-Options: nosniff, X-Frame-Options: DENY\nGET /api/items → Лог: GET /api/items 200 12.3ms\nПри 101-м запросе с одного IP → 429 Too Many Requests, body: {"detail": "Лимит превышен"}, headers: Retry-After: 60\nGET /docs → 200 OK (исключён из rate limit)\nОтвет > 1KB → автоматически сжимается GZip, headers: Content-Encoding: gzip',
