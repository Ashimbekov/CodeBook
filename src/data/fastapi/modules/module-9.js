export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'alembic init alembic → Создаётся директория alembic/ с env.py, alembic.ini\nalembic revision --autogenerate -m "create users table" → Генерируется файл миграции\nalembic upgrade head → INFO: Running upgrade -> abc123, create users table\nalembic current → abc123 (head)\nalembic history → abc123 -> def456 (head), create posts table\nalembic downgrade -1 → INFO: Running downgrade def456 -> abc123\nПосле всех миграций в БД: таблицы users, posts, tags, post_tags с индексами',
