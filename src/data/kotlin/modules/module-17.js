  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'Repository<T, ID> — паттерн с двумя параметрами: T — тип сущности, ID — тип идентификатора (Int, Long, UUID)',
          'item.also { store[it.id] = it } — save возвращает сохранённый элемент (builder-паттерн), also не меняет тип',
          'LinkedHashMap с accessOrder=true (третий параметр) — автоматический LRU: при доступе перемещает в конец',
          'sealed class Result<T> { data class Success<T>(val data: T); data class Error(val e: Exception) } — паттерн для результата',
          'Generics + sealed class + when — мощная комбинация: when(result) { is Success -> ... is Error -> ... } без кастов'
        ]},
        { type: 'tip', value: 'InMemoryRepository — незаменим для юнит-тестов. Вместо мокирования реального репозитория создайте generic InMemoryRepository<T, ID> который работает для любой сущности.' }
      ]
    },
    {
      id: 7,
