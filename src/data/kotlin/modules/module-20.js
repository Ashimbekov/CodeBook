  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'writeText перезаписывает файл целиком — опасно для лог-файлов. Для добавления используй appendText',
          'bufferedWriter().use { } — буферизация ускоряет запись многих строк: IO-вызовы группируются, не по одному на строку',
          'writer.newLine() вместо "\\n" — автоматически использует разделитель строк для текущей ОС (\\r\\n на Windows, \\n на Unix)',
          'printWriter позволяет использовать printf-форматирование: %s для строк, %d для чисел, %n для переноса строки',
          'use { } — аналог Java try-with-resources: гарантирует закрытие потока даже при исключении'
        ]},
        { type: 'tip', value: 'Для атомарной записи важных файлов: записывай во временный файл, затем переименовывай через File("tmp").renameTo(File("target")). Это предотвращает повреждение при сбое во время записи.' }
      ]
    },
    {
      id: 3,
