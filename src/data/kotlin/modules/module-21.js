  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'CoroutineScope(Dispatchers.IO + SupervisorJob()) — SupervisorJob изолирует дочерние: одна упавшая корутина не отменит остальные',
          'coroutineScope { } — suspend-функция, ждёт всех дочерних. Падение одной дочерней отменяет всех и бросает исключение вверх',
          'withTimeout бросает TimeoutCancellationException — это CancellationException, его не нужно явно ловить если только нужна обработка',
          'withTimeoutOrNull — более безопасная альтернатива: возвращает null вместо исключения при таймауте',
          'scope.cancel() — отменяет все дочерние корутины. Обязательно вызывать при уничтожении компонента (onDestroy, onCleared)'
        ]},
        { type: 'tip', value: 'CoroutineScope в классах всегда создавай с SupervisorJob: CoroutineScope(Dispatchers.IO + SupervisorJob()). Без SupervisorJob падение одной корутины уничтожит весь scope.' }
      ]
    },
    {
      id: 8,
