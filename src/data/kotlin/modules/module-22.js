  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'Без buffer: producer ждёт consumer после каждого emit — итого N*(producer_delay + consumer_delay)',
          'С buffer: producer и consumer работают параллельно — итого max(producer_time, consumer_time)',
          'conflate похож на buffer(0, DROP_OLDEST) — при медленном consumer пропускает все кроме последнего значения',
          'debounce(timeMs): emit только если после последнего значения прошло timeMs мс тишины — идеален для поиска с вводом',
          'collectLatest: отменяет обработку предыдущего при новом — похоже на debounce но для долгой обработки, не задержки ввода'
        ]},
        ...
        { type: 'tip', value: 'Паттерн поиска с вводом: queryFlow.debounce(300).distinctUntilChanged().flatMapLatest { searchAPI(it) }. Три оператора вместе — debounce убирает частые обновления, distinctUntilChanged — дубликаты, flatMapLatest — отменяет устаревшие запросы.' }
      ]
    },
    {
      id: 7,
