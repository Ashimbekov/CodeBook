  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'Вся стандартная библиотека Kotlin — это extensions: map, filter, forEach, sortedBy — всё добавлено к Iterable через extensions',
          'fun <T> List<T>.second() — generic extension: работает с любым типом, не только Int или String',
          'Extension для Map с lambda-default: () -> V вместо V — default вычисляется лениво, только если ключа нет',
          'Iterable vs Collection vs List — разные уровни абстракции. Определяй extension на наименее специфичном типе который нужен',
          'forEachIndexedFrom показывает паттерн: если нужен счётчик — используй var index внутри extension, не передавай его снаружи'
        ]},
        { type: 'tip', value: 'Создавай extensions для коллекций в файле CollectionExtensions.kt или аналогичном. Группировка по теме делает код находимым — разработчики сначала ищут в stdlib, потом в вашем extensions-файле.' }
      ]
    },
    {
      id: 4,
