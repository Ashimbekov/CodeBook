  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Практические паттерны с groupby' },
        { type: 'code', language: 'python', value: 'import pandas as pd\nimport numpy as np\n\ndf = pd.DataFrame({\n    \'клиент\': [\'А\', \'Б\', \'А\', \'В\', \'Б\', \'А\', \'В\'],\n    \'покупка\': [100, 200, 150, 300, 250, 180, 120],\n    \'дата\': pd.date_range(\'2024-01-01\', periods=7)\n})\n\n# Кумулятивная сумма по группам\ndf[\'накопл_покупки\'] = df.groupby(\'клиент\')[\'покупка\'].cumsum()\n\n# Порядковый номер в группе\ndf[\'номер_покупки\'] = df.groupby(\'клиент\').cumcount() + 1\n\n# Первая и последняя дата для каждого клиента\nfirst_last = df.groupby(\'клиент\').agg(\n    первая_покупка=(\'дата\', \'min\'),\n    последняя_покупка=(\'дата\', \'max\'),\n    всего_покупок=(\'покупка\', \'count\'),\n    общая_сумма=(\'покупка\', \'sum\')\n)\nprint(first_last)\n\n# named aggregation (pandas 0.25+)\nresult = df.groupby(\'клиент\').agg(\n    средний_чек=(\'покупка\', \'mean\'),\n    макс=(\'покупка\', \'max\')\n)\nprint(result)' },
        { type: 'tip', value: 'Named aggregation (agg с именованными парами) даёт чистые имена столбцов без MultiIndex. Формат: новое_имя=(столбец, функция). Доступно с Pandas 0.25+.' }
      ]
    },
    {
      id: 3,
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'pd.crosstab и работа с результатами pivot_table' },
        { type: 'code', language: 'python', value: 'import pandas as pd\n\n# pd.crosstab — упрощённый вариант для частот\ndf = pd.DataFrame({\n    \'пол\': [\'М\', \'Ж\', \'М\', \'Ж\', \'М\', \'Ж\', \'М\'],\n    \'отдел\': [\'IT\', \'HR\', \'IT\', \'IT\', \'HR\', \'Finance\', \'Finance\']\n})\n\nct = pd.crosstab(df[\'пол\'], df[\'отдел\'])\nprint(ct)\n# отдел  Finance  HR  IT\n# пол\n# Ж            1   1   1\n# М            1   1   2\n\n# normalize — в процентах\nprint(pd.crosstab(df[\'пол\'], df[\'отдел\'], normalize=\'index\'))\n# каждая строка в долях 1.0\n\n# Работа с результатами pivot_table\npivot = pd.pivot_table(\n    df, values=None, index=\'пол\', columns=\'отдел\',\n    aggfunc=len, fill_value=0\n)\n# Сбросить иерархию столбцов\npivot.columns = [str(c) for c in pivot.columns]\n# Преобразовать обратно в "длинный" формат\npivot_long = pivot.stack().reset_index()\npivot_long.columns = [\'пол\', \'отдел\', \'количество\']' },
        { type: 'note', value: 'pd.crosstab — это упрощённая версия groupby+pivot_table для подсчёта частот. Используй crosstab для быстрой разведочной аналитики, pivot_table для более сложных агрегаций.' }
      ]
    },
    {
      id: 5,
      ...
      content: [
        ...
        { type: 'heading', value: 'Когда использовать широкий vs длинный формат' },
        { type: 'code', language: 'python', value: 'import pandas as pd\n\n# Длинный формат удобен для:\n# 1. Группировки (groupby)\n# 2. Построения графиков (seaborn предпочитает long format)\n# 3. Фильтрации по значениям одного столбца\n\ndf_long = pd.DataFrame({\n    \'студент\': [\'Алиса\', \'Алиса\', \'Боб\', \'Боб\'],\n    \'предмет\': [\'Математика\', \'Физика\', \'Математика\', \'Физика\'],\n    \'оценка\': [90, 85, 78, 92]\n})\n\n# Удобно: средняя оценка по предмету\nprint(df_long.groupby(\'предмет\')[\'оценка\'].mean())\n\n# Удобно: только оценки > 85\nprint(df_long[df_long[\'оценка\'] > 85])\n\n# Широкий формат удобен для:\n# 1. Отображения (человеку легче читать таблицу)\n# 2. Вычислений с несколькими столбцами одновременно\n# 3. Корреляционного анализа\n\ndf_wide = df_long.pivot(index=\'студент\', columns=\'предмет\', values=\'оценка\')\nprint(df_wide)\n#             Математика  Физика\n# студент\n# Алиса               90      85\n# Боб                 78      92\n\n# Удобно: корреляция между предметами\nprint(df_wide.corr())' },
        { type: 'tip', value: 'Общее правило: храни данные в длинном формате для анализа (groupby, seaborn), конвертируй в широкий для отображения или когда нужны вычисления между несколькими показателями.' }
      ]
    },
    {
      id: 6,
