  ...
  lessons: [
    {
      ...
      content: [
        ...
        { type: 'tip', value: 'Хитрость запоминания: группируйте глаголы по паттерну изменения: write→wrote, drive→drove, ride→rode (одинаковое изменение). Run→ran, come→came, become→became (похожий паттерн). Это легче, чем учить каждый отдельно.' }
      ]
    },
    {
      id: 2,
      ...
      content: [
        ...
        { type: 'tip', value: 'Запомните пары: write/wrote (писать/написал), run/ran (запускать/запустил), find/found (находить/нашёл), build/built (создавать/создал), make/made (делать/сделал), read/read (читать/прочитал — произносится [red]!). Используйте их каждый день.' },
      ]
    },
    {
      id: 3,
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'note', value: 'В Git commit messages используйте прошедшее время: "Fixed authentication bug", "Added user validation", "Removed deprecated code", "Updated dependencies". Это стандартный стиль коммит-сообщений в IT-сообществе.' },
      ]
    },
    {
      id: 6,
      ...
      content: [
        ...
        { type: 'tip', value: 'Стратегия запоминания: учите глаголы в контексте IT-ситуаций. "Found a bug → fixed it → sent the PR → got feedback → made changes → chose to merge." Один связный рассказ с несколькими глаголами запоминается лучше, чем изолированный список.' },
      ]
    },
    {
      id: 7,
