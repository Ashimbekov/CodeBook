  ...
  lessons: [
    {
      ...
      expectedOutput: 'kubectl get all -l app=static-site:\nNAME                READY   STATUS    RESTARTS\npod/static-site-1   1/1     Running   0\npod/static-site-2   1/1     Running   0\n\nminikube service static-site --url:\nhttp://192.168.64.2:31234\n\ncurl http://192.168.64.2:31234:\n<!DOCTYPE html>\n<html><head><title>My K8s Site</title></head>\n<body><h1>Hello from Kubernetes!</h1></body></html>',
    ...
    },
    {
      ...
      expectedOutput: 'kubectl get all:\nNAME                        READY   STATUS    RESTARTS\npod/mongodb-0               1/1     Running   0\npod/nodejs-api-xxx-1        1/1     Running   0\npod/nodejs-api-xxx-2        1/1     Running   0\n\nkubectl get pvc:\nNAME                      STATUS   CAPACITY\nmongo-data-mongodb-0      Bound    5Gi\n\nReadiness probe проходит: все nodejs-api Pod готовы к трафику.',
    ...
    },
    {
      ...
      expectedOutput: 'kubectl get all:\nПоды: redis-0, backend-xxx-1, backend-xxx-2, frontend-xxx-1, frontend-xxx-2 все Running.\n\ncurl http://$MINIKUBE_IP/:\nNginx приветственная страница (frontend)\n\ncurl http://$MINIKUBE_IP/api:\nBackend: hits=1\n\ncurl http://$MINIKUBE_IP/api повторно:\nBackend: hits=2',
    ...
    },
    {
      ...
      expectedOutput: 'До переключения:\nwget -qO- http://app-service:\nBlue version v1.0\n\nПосле kubectl patch service app-service ... version: green:\nwget -qO- http://app-service:\nGreen version v2.0\n\nПосле отката (version: blue):\nBlue version v1.0\n\nПереключение мгновенное — без downtime.',
    ...
    },
    {
      ...
      expectedOutput: '20 запросов к canary-service:\nStable v1.0 x18\nCanary v2.0 x2\n(~10% трафика на canary)\n\nПосле масштабирования до 50/50:\nStable v1.0 x10\nCanary v2.0 x10\n\nПосле полного переключения (stable=0, canary=10):\nCanary v2.0 x20',
    ...
    },
    {
      ...
      expectedOutput: 'kubectl get pods:\nNAME                       READY   STATUS\nmysql-xxx                  1/1     Running\nwordpress-xxx              1/1     Running\n\nkubectl get pvc:\nmysql-pvc       Bound   5Gi\nwordpress-pvc   Bound   2Gi\n\nminikube service wordpress-service --url:\nhttp://192.168.64.2:30080\n\nhttp://192.168.64.2:30080 открывает мастер установки WordPress.',
    ...
    },
    {
      ...
      expectedOutput: 'До генерации нагрузки:\nkubectl get hpa:\nNAME             TARGETS   MINPODS   MAXPODS   REPLICAS\nphp-apache-hpa   5%/50%    1         10        1\n\nПосле запуска load-generator (через 1-2 минуты):\nkubectl get hpa:\nNAME             TARGETS    MINPODS   MAXPODS   REPLICAS\nphp-apache-hpa   214%/50%   1         10        5\n\nПосле остановки нагрузки (через 5 минут): REPLICAS вернулось к 1.',
    ...
    },
    {
      ...
      expectedOutput: 'kubectl logs -l job-name=pipeline-step1-download:\nStep 1: Downloading data...\nDownloaded 100 records\n100\n\nkubectl logs -l job-name=pipeline-step2-process:\nStep 2: Processing...\nProcessing complete:\nTotal: 49832 Average: 498.32\n\nkubectl get jobs:\npipeline-step1-download   1/1   5s\npipeline-step2-process    1/1   3s',
    ...
    },
    {
      ...
      expectedOutput: 'kubectl get pods -A | grep app:\ndev          app-xxx-1     1/1   Running\nstaging      app-xxx-1     1/1   Running\nstaging      app-xxx-2     1/1   Running\nproduction   app-xxx-1     1/1   Running\nproduction   app-xxx-2     1/1   Running\nproduction   app-xxx-3     1/1   Running\n\nCross-namespace запрос из dev:\nwget -qO- http://app-service.production.svc.cluster.local:\nEnv: production, Replicas: 3',
    ...
    },
    {
      ...
      expectedOutput: 'kubectl get pod broken-image:\nNAME           READY   STATUS              RESTARTS\nbroken-image   0/1     ImagePullBackOff    0\nDiagностика: Failed to pull image "nginx:99.99-nonexistent": not found\n\nkubectl get pod crashloop-pod:\nNAME            READY   STATUS             RESTARTS\ncrashloop-pod   0/1     CrashLoopBackOff   3\nДиагностика: Container exited with code 1\n\nkubectl describe pod pending-pod:\nEvents: 0/1 nodes are available: 1 node(s) didn\'t match node selector.\n\nkubectl get endpoints broken-service:\nNAME             ENDPOINTS   AGE\nbroken-service   <none>      30s\nПосле исправления selector: ENDPOINTS показывает IP Pod.',
