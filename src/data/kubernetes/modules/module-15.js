export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'kubectl get pods -n monitoring:\nNAME                                        READY   STATUS\nmonitoring-grafana-xxx                      3/3     Running\nmonitoring-prometheus-xxx                   2/2     Running\nmonitoring-kube-state-metrics-xxx           1/1     Running\nmonitoring-prometheus-node-exporter-xxx     1/1     Running\n\nGrafana http://localhost:3000 — вход admin/admin123.\nPrometheus http://localhost:9090 -> Status -> Targets: demo-app UP.\n\nServiceMonitor создан: Prometheus обнаружил demo-app как цель за ~15 секунд.',
    ...
    },
    {
      ...
      expectedOutput: 'PromQL запрос: sum(rate(container_cpu_usage_seconds_total{container!=""}[5m])) by (namespace)\nВозвращает: {namespace="default"} 0.002, {namespace="monitoring"} 0.05\n\nPromQL: (node_filesystem_avail_bytes{mountpoint="/"} / node_filesystem_size_bytes{mountpoint="/"}) * 100\nВозвращает: 73.4 (73.4% свободного места)\n\nPrometheusRule загружен: Prometheus UI -> Rules показывает группу memory.rules.\nАлерт HighMemoryUsage появляется в Alertmanager при превышении 80% памяти.',
