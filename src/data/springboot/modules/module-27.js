export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'POST http://localhost:8081/api/orders (Order Service):\n{\n  "userId": 1,\n  "items": ["item1", "item2"]\n}\n\nOrder Service запрашивает данные пользователя через Feign:\nGET http://localhost:8082/api/users/1 — 200 OK\n{"id":1,"name":"Алибек","address":"Алматы, ул. Абая 1"}\n\nЗаказ сохранён в БД. Событие опубликовано в Kafka топик "orders":\n{"orderId":42,"userId":1}\n\nNotification Service получает событие:\nINFO  OrderConsumer: Новый заказ: orderId=42, userId=1\n\nЕсли User Service недоступен (Circuit Breaker):\nWARN  User Service недоступен, используем кешированные данные\nЗаказ создан без данных адреса пользователя.\nCircuit Breaker статус: OPEN (после 5 неудачных попыток)',
