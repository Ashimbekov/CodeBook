  ...
  lessons: [
    {
      ...
      expectedOutput: 'POST /auth/register (без аутентификации):\nHTTP 201 Created — разрешено для всех\n\nGET /api/posts (без аутентификации):\nHTTP 200 OK — публичный доступ\n\nPOST /api/posts (без токена):\nHTTP 401 Unauthorized\n\nGET /api/profile (без токена):\nHTTP 401 Unauthorized\n\nPOST /api/posts с заголовком Authorization: Bearer <valid_jwt>:\nHTTP 201 Created — аутентифицированный пользователь\n\nGET /actuator/health (без токена):\nHTTP 200 OK — публичный endpoint\n\nSESSION не создаётся (STATELESS) — в логах нет "Creating new HTTP session".',
    ...
    },
    {
      ...
      expectedOutput: 'POST /auth/register {"name":"Дана","email":"dana@mail.ru","password":"Password1!"}:\nHTTP 201 Created\n{"accessToken":"eyJhbGciOiJIUzI1NiJ9...","refreshToken":"a1b2c3d4-...","expiresIn":900}\n\nPOST /auth/login {"email":"dana@mail.ru","password":"Password1!"}:\nHTTP 200 OK\n{"accessToken":"eyJ...новый_токен...","refreshToken":"e5f6...","expiresIn":900}\n\nPOST /auth/register (уже существующий email):\nHTTP 409 Conflict\n{"error":"Email dana@mail.ru уже используется"}\n\nPOST /auth/refresh {"refreshToken":"e5f6..."}:\nHTTP 200 OK\n{"accessToken":"eyJ...обновлённый...","refreshToken":"новый-uuid","expiresIn":900}\nСтарый refresh токен удалён из БД (rotation).\n\nPOST /auth/logout:\nHTTP 204 No Content\nRefresh токен удалён из БД.',
    ...
    },
    {
      ...
      expectedOutput: 'GET /admin/users (роль USER):\nHTTP 403 Forbidden\n{"error":"Access Denied"}\n\nGET /admin/users (роль ADMIN):\nHTTP 200 OK\n[{"id":1,"name":"Алибек"},{"id":2,"name":"Дана"}]\n\nDELETE /api/posts/1 (USER, не автор поста):\nHTTP 403 Forbidden\n\nDELETE /api/posts/1 (USER, автор поста — isOwner=true):\nHTTP 204 No Content\n\nDELETE /api/posts/1 (ADMIN, не автор):\nHTTP 204 No Content\n\nPUT /api/posts/2/ban (MODERATOR):\nHTTP 200 OK\n{"id":2,"status":"BANNED"}\n\nPUT /api/posts/2/ban (USER):\nHTTP 403 Forbidden',
    ...
    },
    {
      ...
      expectedOutput: 'Пользователь переходит на GET /oauth2/authorization/google.\nBrowser редиректируется на accounts.google.com для аутентификации.\n\nПосле успешного входа Google редиректирует на /login/oauth2/code/google.\n\nCustomOAuth2UserService вызывается:\n- email: alibek@gmail.com\n- name: Алибек Жаксыбеков\n- sub: 1234567890\n- Пользователь alibek@gmail.com не найден — создан новый аккаунт\n  INSERT INTO users (name, email, google_id, avatar_url) VALUES (...)\n\nOAuth2SuccessHandler генерирует JWT и редиректирует:\nHTTP 302 Location: https://myfrontend.kz/oauth2/callback?token=eyJhbGciOiJIUzI1NiJ9...\n\nПри повторном входе того же пользователя:\n- Пользователь alibek@gmail.com найден — обновлён googleId и avatarUrl',
    ...
    },
    {
      ...
      expectedOutput: 'POST /auth/2fa/enable:\nHTTP 200 OK\n{\n  "secret": "JBSWY3DPEHPK3PXP",\n  "qrUri": "otpauth://totp/MyApp:dana@mail.ru?secret=JBSWY3DPEHPK3PXP&issuer=MyApp"\n}\nQR код сканируется в Google Authenticator.\n\nPOST /auth/2fa/verify {"code":"123456"}:\nHTTP 200 OK — 2FA включена для аккаунта\n\nPOST /auth/2fa/verify {"code":"000000"} (неверный код):\nHTTP 400 Bad Request\n{"error":"Неверный код"}\n\nPOST /auth/login (с включённой 2FA):\nHTTP 200 OK\n{"requiresTwoFactor":true,"tempToken":"tmp_eyJ..."}\n\nPOST /auth/2fa/validate {"tempToken":"tmp_eyJ...","code":"654321"}:\nHTTP 200 OK\n{"accessToken":"eyJ...полный_токен...","refreshToken":"..."}\n\nPOST /auth/2fa/disable:\nHTTP 204 No Content — 2FA отключена.',
    ...
    },
    {
      ...
      expectedOutput: 'POST /auth/forgot-password {"email":"dana@mail.ru"}:\nHTTP 200 OK\n{"message":"Если email существует, письмо отправлено"}\nINFO EmailService: Письмо для сброса пароля отправлено на dana@mail.ru\n\nPOST /auth/forgot-password {"email":"несуществующий@mail.ru"}:\nHTTP 200 OK\n{"message":"Если email существует, письмо отправлено"}\n(письмо не отправляется, но ответ идентичный — защита от перебора)\n\nGET /auth/reset-password?token=<valid-uuid>:\nHTTP 200 OK\n{"valid":true}\n\nGET /auth/reset-password?token=<истёкший>:\nHTTP 400 Bad Request\n{"error":"Токен недействителен или истёк"}\n\nPOST /auth/reset-password {"token":"<valid>","newPassword":"NewPass1!"}:\nHTTP 200 OK\nПароль обновлён. Все refresh токены пользователя удалены (все сессии завершены).',
    ...
    },
    {
      ...
      expectedOutput: 'POST /api/keys {"name":"CI Pipeline","permissions":["read","write"]}:\nHTTP 201 Created\n{\n  "id": 1,\n  "name": "CI Pipeline",\n  "key": "sk_a1b2c3d4e5f6...32символа"\n}\n(ключ отображается один раз, потом недоступен)\n\nGET /api/products с заголовком X-API-Key: sk_a1b2c3d4e5f6...:\nApiKeyFilter находит хеш SHA-256 в БД -> аутентификация успешна.\nHTTP 200 OK\napi_keys.last_used обновлён до текущего времени.\n\nDELETE /api/products/1 с ключом с permission "read":\nHTTP 403 Forbidden — только GET запросы.\n\nDELETE /api/keys/1:\nHTTP 204 No Content — ключ отозван (active=false).\nПовторный запрос с отозванным ключом:\nHTTP 401 Unauthorized.',
    ...
    },
    {
      ...
      expectedOutput: 'Первые 4 неудачные попытки с IP 10.0.0.1:\nPOST /auth/login {"email":"x","password":"wrong"}\nHTTP 401 Unauthorized — неверные данные\n\nПятая неудачная попытка:\nWARN LoginAttemptService: IP заблокирован после 5 попыток: 10.0.0.1\nПоследующие запросы с 10.0.0.1:\nHTTP 429 Too Many Requests\nRetry-After: 900\n{"error":"IP заблокирован. Попробуйте через 15 минут."}\n\nGET /admin/blocked-ips (ADMIN):\n["10.0.0.1"]\n\nPOST /admin/unblock/10.0.0.1 (ADMIN):\nHTTP 204 No Content — IP разблокирован\n\nЧерез 15 минут TTL Redis истекает — IP автоматически разблокируется.\n\nGET /admin/blocked-ips (USER):\nHTTP 403 Forbidden',
    ...
    },
    {
      ...
      expectedOutput: 'PostControllerSecurityTest:\n  publicListIsAccessibleWithoutAuth() PASSED — статус 200\n  createRequiresAuth() PASSED — статус 401\n  authenticatedUserCanCreate() PASSED — статус 201 с @WithMockUser\n  adminCanDelete() PASSED — статус 204 с @WithMockUser(roles="ADMIN")\n\nAdminControllerSecurityTest:\n  userCannotAccessAdmin() PASSED — статус 403 с @WithMockUser(roles="USER")\n  adminCanAccessAdmin() PASSED — статус 200 с @WithMockUser(roles="ADMIN")\n\nTests run: 6, Failures: 0, Errors: 0, Skipped: 0\nTime: 1.8 seconds\n\n@WebMvcTest загружает только слой контроллеров (не весь контекст) — тесты быстрые.',
    ...
    },
    {
      ...
      expectedOutput: 'AuthFlowIT с Testcontainers PostgreSQL:\n\nOrder(1) registerUser() PASSED\n  POST /auth/register -> HTTP 201, accessToken и refreshToken получены\n\nOrder(2) accessProtectedWithToken() PASSED\n  GET /api/profile с Bearer токеном -> HTTP 200\n\nOrder(3) accessWithoutToken_Returns401() PASSED\n  GET /api/profile без токена -> HTTP 401\n\nOrder(4) refreshToken() PASSED\n  POST /auth/refresh -> HTTP 200, новый refreshToken\n  Старый refresh токен заменён (rotation)\n\nOrder(5) oldRefreshTokenRejected() PASSED\n  POST /auth/refresh со старым токеном -> HTTP 401\n\nOrder(6) logout() PASSED\n  POST /auth/logout -> HTTP 204\n\nOrder(7) tokenAfterLogout_Returns401() PASSED\n  GET /api/profile с инвалидированным токеном -> HTTP 401\n\nTests run: 7, Failures: 0, Errors: 0, Skipped: 0\nTime: 15.3 seconds',
