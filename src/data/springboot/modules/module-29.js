export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'GET /actuator/health:\n{\n  "status": "UP",\n  "components": {\n    "db": {"status": "UP"},\n    "redis": {"status": "UP"},\n    "externalApi": {"status": "UP", "details": {"api": "external.kz"}}\n  }\n}\n\nGET /actuator/metrics/orders:\n{\n  "name": "orders",\n  "measurements": [\n    {"statistic": "COUNT", "value": 42}\n  ],\n  "availableTags": [\n    {"tag": "status", "values": ["success", "error"]}\n  ]\n}\n\nGET /actuator/prometheus:\norders_total{status="success"} 42.0\norders_total{status="error"} 3.0\norder_processing_seconds_count 45.0\norder_processing_seconds_sum 12.345\n\nPrometheus scraping каждые 15 секунд:\nINFO  Scraping http://app:8080/actuator/prometheus\nGrafana дашборд показывает: RPS, ошибки, время обработки заказов.',
