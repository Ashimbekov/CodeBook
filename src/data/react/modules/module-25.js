export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'Компонент разбит: UserInfo, UserStats, UserPosts — отдельные компоненты\nUserStats мемоизирован через React.memo\nuseUserData(userId) — кастомный хук для загрузки данных\nUsePosts мемоизированы через useMemo\nuseFetchUser(id) отменяет запрос при смене id',
