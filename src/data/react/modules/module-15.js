export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'AuthProvider оборачивает приложение и предоставляет контекст\nlogin(email, password) -> обновляет user в контексте\nlogout() -> обнуляет user\nProtectedRoute перенаправляет на /login если не авторизован\nuseAuth() возвращает { user, login, logout }\nHeader показывает имя пользователя или кнопку входа',
