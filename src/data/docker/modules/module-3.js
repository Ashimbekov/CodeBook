export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'REPOSITORY    TAG       IMAGE ID       CREATED       SIZE\nnginx         alpine    abc123def456   2 days ago    23.4MB\nubuntu        22.04     def456abc789   2 weeks ago   77.9MB\n\nPostgreSQL контейнер запущен, в логах: database system is ready to accept connections.\n\nВнутри psql:\nCREATE TABLE\nINSERT 0 1\n id | name\n----+------\n  1 | Алия\n(1 row)\n\nФайл pg.conf скопирован на хост.\n\ndocker stats --no-stream:\nNAME           CPU %   MEM USAGE / LIMIT\nmy-postgres    0.01%   45MiB / 7.7GiB\n\nmyapp:v1.0 тегирован, сохранён в myapp_v1.tar, удалён и восстановлен:\nLoaded image: myapp:v1.0',
    ...
    },
    {
      ...
      expectedOutput: 'Три тега myapp:latest, myapp:1.0.0, myapp:stable указывают на один IMAGE ID.\n\nОбраз с датой создан: myapp:2026-03-21\n\ndocker images nginx:\nREPOSITORY   TAG        IMAGE ID       SIZE\nnginx        latest     abc123         187MB\nnginx        alpine     def456         23.4MB  <- рекомендуется для prod\nnginx        bookworm   ghi789         187MB\n\nСкрипт cleanup_images.sh создан и удаляет образы с датой старше 7 дней.',
