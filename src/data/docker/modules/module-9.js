export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'docker compose up -d (основной стек):\nNAME              STATUS\nmyapp-postgres-1  Up (healthy)\nmyapp-redis-1     Up (healthy)\nmyapp-app-1       Up\nmyapp-nginx-1     Up\n\ndocker compose --profile dev up -d (добавляет pgadmin):\nmyapp-pgadmin-1   Up   0.0.0.0:5050->80/tcp\n\ndocker inspect postgres --format "{{.State.Health.Status}}":\nhealthy\n\nРесурсные лимиты установлены: postgres ограничен до 512M, redis до 128M.',
