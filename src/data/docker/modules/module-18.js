  ...
  lessons: [
    {
      ...
      expectedOutput: 'docker compose ps:\nNAME               STATUS\nmyapp-mysql-1      Up (healthy)\nmyapp-wordpress-1  Up              0.0.0.0:8080->80/tcp\n\nhttp://localhost:8080 открывает мастер установки WordPress.\nMySQL не публикует порт — недоступен снаружи.\nVolumes mysql_data и wordpress_uploads созданы и сохраняют данные.',
    ...
    },
    {
      ...
      expectedOutput: 'docker compose up -d — сервисы запущены.\n\nИзменение server.js и сохранение файла:\n[nodemon] restarting due to changes...\n[nodemon] starting `node server.js`\n\nAdminer: http://localhost:8080 — веб-интерфейс для PostgreSQL.\nRedis Commander: http://localhost:8081 — веб-интерфейс для Redis.\nDEBUG=* в ENV обеспечивает подробное логирование.',
    ...
    },
    {
      ...
      expectedOutput: './deploy.sh dev — запускает docker compose с dev конфигурацией, bind mounts и debug портами.\n./deploy.sh prod — запускает с prod конфигурацией: ресурсные лимиты, restart: unless-stopped.\n\ndocker compose -f docker-compose.yml -f docker-compose.prod.yml config\nвыводит итоговую YAML конфигурацию без ошибок.\n\n./deploy.sh invalid — выводит: Usage: deploy.sh dev|staging|prod',
    ...
    },
    {
      ...
      expectedOutput: 'docker compose up -d:\nПорядок запуска: postgres -> redis -> migrations -> app -> nginx-proxy\n\ndocker compose ps:\nNAME               STATUS\npostgres           Up (healthy)\nredis              Up (healthy)\nmigrations         Exited (0)    <- успешно завершился\napp                Up (healthy)\nnginx-proxy        Up\n\nПовторный docker compose up:\nmigrations имеет статус Exited (0) — condition: service_completed_successfully выполнен, app запускается без повторных миграций.',
    ...
    },
    {
      ...
      expectedOutput: 'docker compose up -d:\nNAME             STATUS\nnginx            Up  0.0.0.0:80->80/tcp\nfrontend         Up (healthy)\nbackend          Up (healthy)\ncelery-worker    Up\npostgres         Up (healthy)\nredis            Up (healthy)\n\nВсе секреты читаются из /run/secrets/ — в ENV не отображаются.\nРесурсные лимиты соблюдаются: backend 512M, worker 256M, postgres 1G.',
    ...
    },
    {
      ...
      expectedOutput: 'docker compose up -d:\nNAME       STATUS\npostgres   Up (healthy)\nredis      Up (healthy)\napp        Up  0.0.0.0:3000->3000/tcp\n\ncurl http://localhost:3000:\n{"status": "ok"}\n\nПроблемы исправлены:\n- Пароль перенесён в secrets (не виден в docker inspect).\n- Добавлен healthcheck для postgres и redis.\n- Добавлен depends_on с condition: service_healthy.\n- Добавлен internal: true для backend сети.',
