export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'REPOSITORY   TAG     IMAGE ID   SIZE\nts-single    latest  abc123     300MB\nts-multi     latest  def456     110MB\n\ndocker run --rm ts-multi ls node_modules | grep typescript:\n(пустой вывод — typescript не установлен в production образе)\n\ndocker run --rm ts-single ls node_modules | grep typescript:\ntypescript\n\nПосле изменения server.ts, повторная сборка ts-multi:\nStage 2 (production) использует кэш npm ci — пересобирается только COPY dist.',
    ...
    },
    {
      ...
      expectedOutput: 'REPOSITORY   TAG    SIZE\napp          bad    1.5GB\napp          good   200MB\n\nTrivy app:bad:\n2026-03-21T10:00:00Z INFO Detected OS: debian\nTotal: 47 (CRITICAL: 12, HIGH: 23)\n\nTrivy app:good:\nTotal: 3 (CRITICAL: 0, HIGH: 1)\n\nИтог: образ уменьшен в 7 раз, количество критических уязвимостей снижено до 0.',
