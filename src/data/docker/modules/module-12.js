export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'REPOSITORY         TAG     SIZE\nmyapp-bad          latest  1.2GB\nmyapp-optimized    latest  180MB\n\ndocker history myapp-optimized показывает слои:\n 0B    COPY . . (только код)\n 45MB  COPY site-packages (из builder stage)\n 130MB FROM python:3.11-slim (базовый образ)\n\nПовторная сборка после изменения app.py:\nUsing cache — шаг pip install взят из кэша.\nreal 0m3.450s вместо 0m45s при полной сборке.',
