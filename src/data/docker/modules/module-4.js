export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'REPOSITORY   TAG     IMAGE ID   SIZE\nnode-demo    alpine  abc123     85MB\nnode-demo    full    def456     1.09GB\n\ncurl http://localhost:3000/:\n{"message": "Node.js in Docker!", "env": "production"}\n\ndocker exec node-app whoami:\nnode\n\ndocker exec node-app node -e "console.log(process.env.NODE_ENV)":\nproduction',
    ...
    },
    {
      ...
      expectedOutput: 'Первая сборка (без кэша):\nreal 0m45.123s — скачивает образ и устанавливает пакеты.\n\nПосле изменения app.py, повторная сборка:\nreal 0m1.234s — слой с pip install взят из кэша!\n\nВывод при сборке: Using cache для шагов COPY requirements.txt и RUN pip install.\n\nMakefile создан с целями build, push, clean, run, stop.',
