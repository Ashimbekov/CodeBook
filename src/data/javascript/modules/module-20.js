export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      expectedOutput: 'on("data", handler) -> возвращает emitter для цепочки\nemit("data", payload) -> вызывает все подписанные обработчики\noff("data", handler) -> удаляет конкретный обработчик\nonce("connect", fn) -> fn вызывается только один раз',
