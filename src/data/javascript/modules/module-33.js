  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'readFile с "utf-8" возвращает строку; без кодировки — Buffer (бинарные данные)',
          'mkdir с { recursive: true } создаёт все промежуточные папки, не ошибается если уже существует',
          'readdir с { withFileTypes: true } возвращает Dirent объекты с методами isDirectory(), isFile()',
          'path.join() соединяет части пути, path.extname() возвращает расширение (.js, .json)',
          'Рекурсивный обход дерева директорий — классический пример рекурсии с async/await'
        ]},
        { type: 'tip', value: 'Безопасное чтение JSON: оборачивай в try/catch и возвращай fallback значение. Файл может не существовать, содержать невалидный JSON, или не иметь прав доступа. Всегда обрабатывай все три случая.' }
      ]
    },
    {
      id: 7,
      ...
      expectedOutput: 'readFile("data.txt") -> содержимое файла\nwriteFile("out.txt", data) -> файл создан успешно\ncopyFile("src.txt", "dst.txt") -> файл скопирован\nlistDir("./") -> массив имён файлов в папке',
