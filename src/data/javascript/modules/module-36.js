export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      hint: 'Используйте express.Router() для выноса маршрутов. В POST проверьте title через if (!title). Параметры маршрута приходят как строки — преобразуйте через Number(req.params.id).',
      expectedOutput: 'GET /tasks -> [] (пустой массив)\nPOST /tasks {"title":"Задача"} -> 201 { id: 1, title: "Задача", completed: false }\nGET /tasks/999 -> 404 { error: "Задача не найдена" }\nDELETE /tasks/1 -> { message: "Удалено" }\nСервер выводит: "POST /tasks" при каждом запросе',
      solution: '// routes/tasks.js\nconst express = require("express");\nconst router = express.Router();\n\nlet tasks = [\n  { id: 1, title: "Изучить Express", completed: false },\n  { id: 2, title: "Написать API", completed: true }\n];\nlet nextId = 3;\n\nrouter.get("/", (req, res) => {\n  res.json(tasks);\n});\n\nrouter.get("/:id", (req, res) => {\n  const task = tasks.find(t => t.id === Number(req.params.id));\n  if (!task) return res.status(404).json({ error: "Задача не найдена" });\n  res.json(task);\n});\n\nrouter.post("/", (req, res) => {\n  const { title } = req.body;\n  if (!title) return res.status(400).json({ error: "Title обязателен" });\n  const task = { id: nextId++, title, completed: false };\n  tasks.push(task);\n  res.status(201).json(task);\n});\n\nrouter.put("/:id", (req, res) => {\n  const idx = tasks.findIndex(t => t.id === Number(req.params.id));\n  if (idx === -1) return res.status(404).json({ error: "Задача не найдена" });\n  tasks[idx] = { ...tasks[idx], ...req.body };\n  res.json(tasks[idx]);\n});\n\nrouter.delete("/:id", (req, res) => {\n  const idx = tasks.findIndex(t => t.id === Number(req.params.id));\n  if (idx === -1) return res.status(404).json({ error: "Задача не найдена" });\n  tasks.splice(idx, 1);\n  res.json({ message: "Удалено" });\n});\n\nmodule.exports = router;\n\n// app.js\nconst express = require("express");\nconst tasksRouter = require("./routes/tasks");\nconst app = express();\n\napp.use(express.json());\napp.use((req, res, next) => {\n  console.log(`${req.method} ${req.path}`);\n  next();\n});\napp.use("/tasks", tasksRouter);\napp.use((req, res) => res.status(404).json({ error: "Не найдено" }));\napp.use((err, req, res, next) => res.status(500).json({ error: err.message }));\n\napp.listen(3000, () => console.log("Сервер на порту 3000"));',
      explanation: 'Задачи хранятся в массиве в памяти — при перезапуске теряются, но достаточно для прототипа. Router вынесен в отдельный файл: app.use("/tasks", tasksRouter) автоматически добавляет префикс ко всем маршрутам роутера. Number(req.params.id) необходим, так как параметры пути всегда строки. findIndex возвращает -1 если элемент не найден — это сигнал для ответа 404. Middleware логирования вызывает next() передавая управление следующему обработчику.'
    }
  ]
};
