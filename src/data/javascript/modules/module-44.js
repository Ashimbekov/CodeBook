export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      hint: 'В vite.config.ts используйте resolve.alias для "@" -> "src/". Для proxy: server.proxy с target и changeOrigin. Переменные окружения должны начинаться с VITE_ для доступа в клиентском коде через import.meta.env.',
      expectedOutput: 'import Button from "@/components/Button" -> резолвится в src/components/Button\nfetch("/api/users") -> проксируется на http://localhost:3001/users\nimport.meta.env.VITE_API_URL -> значение из .env файла\nnpm run build -> бандл с минификацией и tree-shaking',
      solution: {
