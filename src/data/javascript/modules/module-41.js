export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      hint: 'Используйте fs.createReadStream() и readline для построчного чтения. Transform stream получает chunk, преобразует и передаёт дальше через this.push(). Не забудьте обработать последнюю строку без символа переноса.',
      expectedOutput: 'Обработка 100000 строк завершена\nОтфильтровано: 45000 строк (age > 30)\nЗаписано в output.csv: 45000 строк\nВремя обработки: ~500ms\nПамять: ~15MB (не зависит от размера файла)',
      solution: {
      ...
      explanation: 'CSVParser накапливает буфер и разбивает по переносам строк — последний элемент после split может быть неполной строкой, поэтому он сохраняется в buffer для следующего чанка. _flush вызывается в конце потока для обработки оставшегося буфера. objectMode: true позволяет передавать объекты вместо Buffer/string. FilterTransform просто не вызывает push для объектов не прошедших предикат. pipeline автоматически закрывает все потоки при ошибке в любом из них — надёжнее чем pipe.'
    }
  ]
};
