export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      hint: 'Группируйте тесты через describe(). Каждый тест — it() с одним конкретным сценарием. Для граничных случаев: пустые входные данные, null, отрицательные числа. Используйте toThrow() для проверки выбрасываемых ошибок.',
      expectedOutput: 'PASS src/utils.test.js\n  validateEmail\n    ✓ возвращает true для корректного email\n    ✓ возвращает false для email без @\n  formatDate\n    ✓ форматирует дату корректно\nTest Suites: 1 passed\nTests: 12 passed\nCoverage: 85%',
      ...
      explanation: 'validateEmail проверяет тип до regex — это защищает от TypeError при null/undefined. groupBy использует reduce для построения объекта-группировки: ключ — значение поля, значение — массив элементов. Тест debounce использует jest.useFakeTimers() — без этого тест занял бы реальные 500мс; jest.runAllTimers() мгновенно выполняет все запланированные таймауты. beforeEach/afterEach гарантируют чистое состояние таймеров для каждого теста. Три вызова debounced без паузы — только последний должен выполниться.'
    }
  ]
};
