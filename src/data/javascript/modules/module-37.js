export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      hint: 'Каждый middleware — функция (req, res, next). Вызов next() передаёт управление дальше, next(err) — обработчику ошибок. Middleware ошибок принимает 4 аргумента: (err, req, res, next).',
      expectedOutput: 'Каждый запрос логируется: "[2024-01-01 12:00:00] GET /api/data"\nZапрос без токена -> 401 { error: "Не авторизован" }\nЗапрос с токеном -> 200 данные\nПревышение лимита -> 429 { error: "Слишком много запросов" }',
      ...
      explanation: 'Morgan подключается дважды: один экземпляр пишет в файл с форматом combined (полная информация), второй — в консоль с форматом dev (цветной вывод). Rate limiter хранит временные метки запросов в Map и фильтрует те, что старше 10 секунд — это скользящее окно. Authenticate проверяет заголовок Authorization через строгое сравнение. Error middleware имеет 4 параметра — Express распознаёт его как обработчик ошибок.'
    }
  ]
};
