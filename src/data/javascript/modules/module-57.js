  ...
  lessons: [
    {
      ...
      hint: 'Цикл: итерируйте с конца строки. Рекурсия: reverseString(str) = str.at(-1) + reverseString(str.slice(0,-1)), базовый случай — пустая строка. Reduce: использует "" как начальное значение и добавляет символы в начало.',
    ...
    },
    {
      ...
      hint: 'Оптимизация: проверяйте делители только до Math.sqrt(n). Числа 0 и 1 — не простые. Чётные числа кроме 2 — не простые. Решето Эратосфена для getPrimes: создайте булев массив и вычеркните составные.',
      expectedOutput: 'isPrime(2) -> true\nisPrime(17) -> true\nisPrime(1) -> false\nisPrime(4) -> false\ngetPrimes(20) -> [2, 3, 5, 7, 11, 13, 17, 19]',
    ...
    },
    {
      ...
      hint: 'Один проход O(n): отслеживайте минимальную цену покупки и максимальную прибыль. Для каждой цены: обновите minPrice, вычислите текущую прибыль и обновите maxProfit. Нельзя продать раньше покупки.',
      expectedOutput: 'maxProfit([7,1,5,3,6,4]) -> 5 (купить за 1, продать за 6)\nmaxProfit([7,6,4,3,1]) -> 0 (цены всегда падают)\nmaxProfit([1,2]) -> 1\nmaxProfit([3,3,3]) -> 0',
    ...
    },
    {
      ...
      hint: 'Используйте стек. Для открывающей скобки — добавьте в стек. Для закрывающей — проверьте что на вершине стека соответствующая открывающая. В конце стек должен быть пустым.',
      expectedOutput: 'isValidBrackets("()[]{}") -> true\nisValidBrackets("([{}])") -> true\nisValidBrackets("(]") -> false\nisValidBrackets("{[}") -> false\nisValidBrackets("") -> true',
    ...
    },
    {
      ...
      hint: 'O(n) решение: используйте Map. Для каждого числа nums[i] проверьте есть ли в Map значение (target - nums[i]). Если есть — нашли пару. Если нет — добавьте nums[i] в Map с индексом i.',
      expectedOutput: 'twoSum([2,7,11,15], 9) -> [0, 1]\ntwoSum([3,2,4], 6) -> [1, 2]\ntwoSum([3,3], 6) -> [0, 1]\nОдно решение гарантировано существует',
    ...
    },
    {
      ...
      hint: 'Для каждого слова создайте ключ — отсортированные символы: "eat".split("").sort().join("") -> "aet". Используйте Map для группировки слов с одинаковым ключом.',
    ...
    },
    {
      ...
      hint: 'Проходите по строке, считайте последовательные одинаковые символы. Добавляйте символ и счётчик (если > 1) в результат. Если сжатая строка не короче оригинала — возвращайте оригинал.',
      expectedOutput: 'compressString("aabcccdddd") -> "a2bc3d4"\ncompressString("abcd") -> "abcd" (не сжимать если не короче)\ncompressString("aaaa") -> "a4"\ncompressString("aaBBcc") -> "a2B2c2"',
    ...
    },
    {
      ...
      hint: 'Поддерживайте четыре границы: top, bottom, left, right. Обходите: слева направо по top, сверху вниз по right, справа налево по bottom, снизу вверх по left. После каждого прохода сужайте границы.',
      expectedOutput: 'spiralOrder([[1,2,3],[4,5,6],[7,8,9]]) -> [1,2,3,6,9,8,7,4,5]\nspiralOrder([[1,2,3,4]]) -> [1,2,3,4]\nspiralOrder([[1],[2],[3]]) -> [1,2,3]\nspiralOrder([[1]]) -> [1]',
    ...
    },
    {
      ...
      hint: 'Инициализируйте left=0 и right=arr.length-1. В цикле: mid = Math.floor((left+right)/2). Если arr[mid] === target — нашли. Если arr[mid] < target — left = mid+1. Иначе — right = mid-1.',
      expectedOutput: 'binarySearch([1,3,5,7,9], 7) -> 3\nbinarySearch([1,3,5,7,9], 1) -> 0\nbinarySearch([1,3,5,7,9], 6) -> -1\nbinarySearch([], 1) -> -1\nbinarySearch([5], 5) -> 0',
      solution: '// Итеративный\nfunction binarySearch(arr, target) {\n  let left = 0;\n  let right = arr.length - 1;\n\n  while (left <= right) {\n    const mid = Math.floor((left + right) / 2);\n    if (arr[mid] === target) return mid;\n    if (arr[mid] < target) left = mid + 1;\n    else right = mid - 1;\n  }\n\n  return -1;\n}\n\n// Рекурсивный\nfunction binarySearchRecursive(arr, target, left = 0, right = arr.length - 1) {\n  if (left > right) return -1;\n  const mid = Math.floor((left + right) / 2);\n  if (arr[mid] === target) return mid;\n  if (arr[mid] < target) return binarySearchRecursive(arr, target, mid + 1, right);\n  return binarySearchRecursive(arr, target, left, mid - 1);\n}\n\nconsole.log(binarySearch([1, 3, 5, 7, 9, 11], 7)); // 3\nconsole.log(binarySearch([1, 3, 5], 4));            // -1\nconsole.log(binarySearchRecursive([2, 4, 6, 8, 10], 6)); // 2',
