export default {
  ...
  lessons: [
    ...
    },
    {
      ...
      hint: 'Socket.IO комнаты: socket.join(room), io.to(room).emit(). Для личных сообщений храните Map<userId, socketId>. При отключении socket.on("disconnect") удаляйте пользователя из всех комнат.',
      expectedOutput: 'Пользователь подключился -> "user:joined" всем в комнате\nСообщение отправлено -> "message:new" всем в комнате\nЛичное сообщение -> "message:private" только получателю\n"typing:start" -> уведомление другим участникам\nОтключение -> обновлённый список онлайн пользователей',
      solution: {
