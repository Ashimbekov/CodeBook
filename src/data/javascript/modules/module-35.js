  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'Заголовки в req.headers всегда в нижнем регистре: "content-type", не "Content-Type"',
          'new URL(req.url, base) — разбирает URL, base нужен потому что req.url — относительный путь',
          'url.searchParams — объект для работы с query string: get(), getAll(), has()',
          'res.setHeader() до writeHead(); res.writeHead() до res.write()/end()',
          'res.end() обязателен — без него соединение остаётся открытым и клиент зависнет'
        ]},
        { type: 'tip', value: 'Порядок вызовов в ответе: setHeader() → writeHead() → write() → end(). После writeHead() нельзя менять заголовки. После end() нельзя ничего писать. Это строгий порядок HTTP-протокола.' }
      ]
    },
    {
      id: 3,
