  ...
  lessons: [
    ...
    },
    {
      ...
      requirements: [
        ...
        'Обосновать выбор SQL или конкретного типа NoSQL',
        'Описать ключевые преимущества выбранной БД для сценария',
        'Указать возможные сочетания БД в одной системе'
      ],
      hint: 'Для каждого сценария задай три вопроса: Как структурированы данные? Как они читаются и записываются? Нужны ли транзакции? Ответы на эти вопросы однозначно укажут на тип БД.',
      expectedOutput: 'Для каждого из пяти сценариев выбрана конкретная БД (PostgreSQL, Redis, MongoDB, Cassandra, Neo4j) с обоснованием через структуру данных и паттерн доступа. Описана возможность комбинировать несколько БД в одной системе.',
    ...
    },
    {
      ...
      requirements: [
        ...
        'Выбрать индексы под конкретные запросы',
        'Решить проблему конкурентного доступа к данным',
        'Объяснить механизм блокировки (pessimistic или optimistic locking)'
      ],
      hint: 'Начни с сущностей и их связей. Для каждой таблицы продумай: какие запросы будут к ней? Под эти запросы создавай индексы. Для конкурентного доступа: pessimistic lock (FOR UPDATE) при высокой конкуренции, optimistic lock при редких конфликтах.',
      expectedOutput: 'Схема включает таблицы с правильными типами данных, первичными и внешними ключами. Индексы соответствуют реальным запросам. Механизм блокировки для конкурентного бронирования реализован корректно (транзакция с FOR UPDATE). UNIQUE constraint защищает от дублирования.',
