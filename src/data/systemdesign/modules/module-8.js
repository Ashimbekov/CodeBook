  ...
  lessons: [
    ...
    },
    {
      ...
      requirements: [
        ...
        'Спроектировать обработку ошибок: retry и Dead Letter Queue',
        'Обеспечить idempotency для критичных операций',
        'Описать схему уведомлений о статусе заказа'
      ],
      hint: 'Синхронно выполняй только то, что необходимо для ответа пользователю (сохранить заказ). Всё остальное — асинхронно через очередь. Каждый consumer должен быть idempotent: проверяй order_id перед обработкой.',
      expectedOutput: 'Архитектура разделена: синхронный путь возвращает ответ за 100 мс. Kafka топик с несколькими consumer groups (инвентарь, оплата, email, склад, аналитика). Retry с exponential backoff и DLQ настроены. Idempotency реализована через проверку order_id в Redis.',
