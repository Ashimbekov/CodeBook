  ...
  lessons: [
    ...
    },
    {
      ...
      requirements: [
        ...
        'Реализовать алгоритм через атомарный Lua скрипт',
        'Объяснить зачем нужна атомарность и как Redis её обеспечивает',
        'Описать масштабирование Redis Cluster для rate limiting'
      ],
      hint: 'Без общего хранилища каждый инстанс считает независимо: реальный лимит превышается в N раз. Redis + Lua скрипт решает обе проблемы: общее состояние и атомарность. Redis однопоточный — Lua скрипт не может быть прерван.',
      expectedOutput: 'Описана проблема локального rate limiting при нескольких инстансах. Спроектирована архитектура с Redis. Приведён Lua скрипт (ZREMRANGEBYSCORE → ZCARD → ZADD). Объяснена атомарность. Указана задержка +0.5–1 мс на запрос. Описан Redis Cluster для масштабирования.',
    ...
    },
    {
      ...
      requirements: [
        ...
        'Спроектировать middleware для проверки лимитов',
        'Добавить заголовки X-RateLimit-* в ответ',
        'Описать мониторинг злоупотреблений через топ-N клиентов по 429'
      ],
      hint: 'Если нужны burst-запросы (легитимные пики активности) — Token Bucket лучше Fixed Window. Кешируй план пользователя в Redis чтобы не ходить в БД при каждом запросе. Lua скрипт обеспечивает атомарность проверки и обновления bucket.',
      expectedOutput: 'Rate limiter спроектирован: выбран Token Bucket (разрешает burst). Тарифные планы хранятся в Redis. Middleware проверяет и обновляет bucket атомарно. Заголовки X-RateLimit-* добавляются в ответ. При превышении — 429 с Retry-After. Мониторинг топ клиентов описан.',
