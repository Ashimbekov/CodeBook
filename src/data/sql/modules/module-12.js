  ...
  lessons: [
    {
      ...
      content: [
        ...
        { type: 'list', items: [
          'Аномалия вставки: нельзя добавить сущность без другой (клиента без заказа)',
          'Аномалия обновления: одни данные нужно менять в нескольких местах — риск несоответствия',
          'Аномалия удаления: удаление одной записи случайно уничтожает другие данные',
          'Нормализация = каждый факт хранится ровно в одном месте (принцип DRY для данных)',
          'Нормализованные данные легче поддерживать, но требуют JOIN при чтении'
        ]},
        { type: 'tip', value: 'Быстрая проверка необходимости нормализации: если для изменения одного факта (email клиента) нужно обновить несколько строк — это признак аномалии обновления. Данные следует нормализовать.' }
      ]
    },
    {
      id: 2,
    ...
    },
    {
      ...
      expectedOutput: 'Исходная денормализованная таблица:\n order_id | customer_name | customer_email | product_name | category    | cat_description | quantity | price\n----------+---------------+----------------+--------------+-------------+-----------------+----------+--------\n        1 | Алия          | aliya@mail.ru  | Ноутбук Dell | Ноутбуки    | Компьютеры      |        1 | 85000\n        2 | Алия          | aliya@mail.ru  | Телефон      | Телефоны    | Мобильные       |        2 | 65000\n\nПосле нормализации до 3NF — созданы таблицы:\n categories: id, name, description\n customers:  id, name, email\n products:   id, name, category_id, price\n orders:     id, customer_id, created_at\n order_items:id, order_id, product_id, quantity, price_at_time\n\nПреимущества: обновление email Алии — 1 UPDATE в customers (было бы N в денормализованной). Аномалий вставки и удаления нет.\n\nДанные перенесены:\nINSERT INTO categories ... (2 rows)\nINSERT INTO customers  ... (1 row)\nINSERT INTO products   ... (2 rows)\nINSERT INTO orders     ... (1 row)\nINSERT INTO order_items ... (2 rows)',
