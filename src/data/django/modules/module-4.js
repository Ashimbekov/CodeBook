  ...
  lessons: [
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Передача дополнительных параметров в view' },
        { type: 'code', language: 'python', value: '# Дополнительные параметры через словарь в path()\nfrom django.urls import path\nfrom . import views\n\nurlpatterns = [\n    # Передать template_name в view как дополнительный аргумент\n    path(\n        "about/",\n        views.static_page,\n        {"template_name": "pages/about.html", "title": "О нас"},\n        name="about"\n    ),\n]\n\n# views.py\ndef static_page(request, template_name, title):\n    return render(request, template_name, {"title": title})\n\n# Кастомный конвертер типов:\nclass FourDigitYearConverter:\n    regex = "[0-9]{4}"\n\n    def to_python(self, value):\n        return int(value)  # конвертация строки в Python-тип\n\n    def to_url(self, value):\n        return "%04d" % value  # конвертация при генерации URL\n\nfrom django.urls import register_converter\nregister_converter(FourDigitYearConverter, "yyyy")\n\n# Теперь можно использовать <yyyy:year> в путях\nurlpatterns += [path("archive/<yyyy:year>/", views.archive)]' },
        { type: 'tip', value: 'Используй path() вместо url() — устаревшая url() убрана в Django 4.0. Для сложных паттернов используй re_path() из django.urls.' }
      ]
    },
    {
      id: 2,
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Вложенные пространства имён' },
        { type: 'code', language: 'python', value: '# API с версиями через вложенные namespace\n# api/urls.py\napp_name = "api"\n\nfrom django.urls import path, include\n\nurlpatterns = [\n    path("v1/", include("api.v1.urls", namespace="v1")),\n    path("v2/", include("api.v2.urls", namespace="v2")),\n]\n\n# api/v1/urls.py\napp_name = "v1"\nurlpatterns = [\n    path("users/", views.user_list, name="user-list"),\n]\n\n# Генерация URL для вложенных namespace:\n# reverse("api:v1:user-list")  -> /api/v1/users/\n# reverse("api:v2:user-list")  -> /api/v2/users/\n\n# В шаблоне:\n# {% url "api:v1:user-list" %}' },
        { type: 'note', value: 'Если app_name не задан в urls.py приложения, Django выдаст ошибку при использовании namespace в include(). Это частая ошибка — всегда добавляй app_name = "имя_приложения" при использовании namespace.' }
      ]
    },
    {
      id: 5,
