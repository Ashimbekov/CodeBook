  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'drf-extensions: кеш декоратор для ViewSet' },
        { type: 'code', language: 'python', value: '# pip install drf-extensions\nfrom rest_framework_extensions.cache.decorators import cache_response\nfrom rest_framework_extensions.key_constructor.constructors import DefaultKeyConstructor\nfrom rest_framework_extensions.key_constructor.bits import QueryParamsKeyBit, UserKeyBit\n\n# Кастомный ключ кеша — зависит от query params и пользователя:\nclass UserQueryParamsKeyConstructor(DefaultKeyConstructor):\n    query_params = QueryParamsKeyBit()\n    user = UserKeyBit()  # разный кеш для каждого пользователя\n\n# Применение:\nclass ArticleViewSet(viewsets.ModelViewSet):\n    queryset = Article.objects.all()\n    serializer_class = ArticleSerializer\n    object_cache_key_func = UserQueryParamsKeyConstructor()\n\n    @cache_response(timeout=60*15)  # кешировать list на 15 минут\n    def list(self, request, *args, **kwargs):\n        return super().list(request, *args, **kwargs)\n\n    @cache_response(timeout=60*60)  # кешировать detail на 1 час\n    def retrieve(self, request, *args, **kwargs):\n        return super().retrieve(request, *args, **kwargs)\n\n# Инвалидация через сигналы:\nfrom django.db.models.signals import post_save, post_delete\nfrom django.dispatch import receiver\n\n@receiver([post_save, post_delete], sender=Article)\ndef invalidate_article_cache(sender, instance, **kwargs):\n    cache.delete_pattern(f"article_viewset_*")' },
        { type: 'note', value: 'cache.delete_pattern() доступен только с django-redis. Если используешь другой бэкенд — храни список ключей вручную или используй cache.clear() (но это очищает весь кеш).' }
      ]
    },
    {
      id: 5,
