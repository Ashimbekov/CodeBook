  ...
  lessons: [
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Finders: где Django ищет статику' },
        { type: 'code', language: 'python', value: '# Django использует Finders для поиска статических файлов:\n# По умолчанию в INSTALLED_APPS:\n# django.contrib.staticfiles.finders.FileSystemFinder  <- STATICFILES_DIRS\n# django.contrib.staticfiles.finders.AppDirectoriesFinder  <- app/static/\n\n# Дополнительные настройки:\nSTATICFILES_FINDERS = [\n    "django.contrib.staticfiles.finders.FileSystemFinder",\n    "django.contrib.staticfiles.finders.AppDirectoriesFinder",\n    # "compressor.finders.CompressorFinder",  # если используешь django-compressor\n]\n\n# Проверить что Django найдёт файл:\n# python manage.py findstatic css/style.css\n# Found \'css/style.css\' here:\n#   /home/user/myproject/static/css/style.css\n\n# Пространство имён для избежания конфликтов между приложениями:\n# blog/static/blog/style.css  -> {% static \'blog/style.css\' %}\n# shop/static/shop/style.css  -> {% static \'shop/style.css\' %}\n\n# Хранение версий (cache busting) через ManifestStaticFilesStorage:\n# STATICFILES_STORAGE = "django.contrib.staticfiles.storage.ManifestStaticFilesStorage"\n# style.css -> style.abc123.css (хэш содержимого в имени)' },
        { type: 'tip', value: 'Всегда используй пространство имён для статики приложений: app/static/app/style.css (не app/static/style.css). Это предотвращает перезапись файлов при collectstatic если два приложения имеют файлы с одинаковым именем.' }
      ]
    },
    {
      id: 2,
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Конвертация в WebP и lazy loading' },
        { type: 'code', language: 'python', value: '# Конвертация в WebP при сохранении (Pillow):\nfrom PIL import Image\nfrom io import BytesIO\nfrom django.core.files.base import ContentFile\nimport os\n\nclass Product(models.Model):\n    image = models.ImageField(upload_to="products/", null=True, blank=True)\n\n    def save(self, *args, **kwargs):\n        if self.image and hasattr(self.image, "file"):\n            # Открываем исходное изображение\n            img = Image.open(self.image)\n            img = img.convert("RGB")  # WebP не поддерживает RGBA\n\n            # Изменяем размер если больше 1200px\n            if img.width > 1200:\n                ratio = 1200 / img.width\n                new_height = int(img.height * ratio)\n                img = img.resize((1200, new_height), Image.LANCZOS)\n\n            # Сохраняем как WebP\n            buffer = BytesIO()\n            img.save(buffer, format="WEBP", quality=85)\n            buffer.seek(0)\n\n            # Меняем имя файла на .webp\n            original_name = os.path.splitext(self.image.name)[0]\n            self.image.save(\n                f"{original_name}.webp",\n                ContentFile(buffer.read()),\n                save=False\n            )\n        super().save(*args, **kwargs)\n\n# В шаблоне с lazy loading:\n# <img src="{{ product.image.url }}"\n#      loading="lazy"\n#      alt="{{ product.name }}"\n#      width="400" height="300">' },
        { type: 'tip', value: 'WebP обеспечивает на 25-35% меньший размер файла чем JPEG при том же качестве. Добавь loading="lazy" к изображениям не в первом экране — браузер загрузит их только когда пользователь доскроллит.' }
      ]
    },
    {
      id: 6,
