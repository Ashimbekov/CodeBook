  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'values(), values_list() и оконные функции' },
        { type: 'code', language: 'python', value: '# values() — словари вместо объектов модели (быстрее)\nArticle.objects.values("id", "title", "author__username")\n# [{"id": 1, "title": "Пост", "author__username": "admin"}, ...]\n\n# values_list() — кортежи\nArticle.objects.values_list("id", "title")\n# [(1, "Пост"), (2, "Другой"), ...]\n\n# Flat для одного поля\nids = Article.objects.values_list("id", flat=True)\n# [1, 2, 3, ...]\n\n# Группировка с annotate + values\nstats = Article.objects.values("category__name").annotate(\n    articles_count=Count("id"),\n    avg_views=Avg("views_count")\n).order_by("-articles_count")\n# [{"category__name": "Python", "articles_count": 15, "avg_views": 245.3}, ...]\n\n# Оконные функции (Django 4.2+):\nfrom django.db.models import Window\nfrom django.db.models.functions import Rank\n\narticles = Article.objects.annotate(\n    rank=Window(\n        expression=Rank(),\n        partition_by=["category"],\n        order_by=F("views_count").desc()\n    )\n)' },
        { type: 'tip', value: 'Используй values() и values_list() когда нужны только данные без объектов модели — это значительно быстрее: меньше памяти, нет создания Python объектов. Особенно полезно для экспорта данных или подготовки данных для сериализации.' }
      ]
    },
    {
      id: 4,
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Несколько менеджеров и soft delete через менеджер' },
        { type: 'code', language: 'python', value: '# Soft delete (мягкое удаление) через кастомный менеджер:\nfrom django.db import models\nfrom django.utils import timezone\n\nclass SoftDeleteQuerySet(models.QuerySet):\n    def delete(self):\n        """Переопределяем удаление — помечаем как удалённые"""\n        return super().update(deleted_at=timezone.now())\n\n    def hard_delete(self):\n        """Реальное удаление из БД"""\n        return super().delete()\n\n    def alive(self):\n        return self.filter(deleted_at__isnull=True)\n\n    def deleted(self):\n        return self.filter(deleted_at__isnull=False)\n\nclass SoftDeleteManager(models.Manager):\n    def get_queryset(self):\n        """По умолчанию показываем только не удалённые"""\n        return SoftDeleteQuerySet(self.model, using=self._db).alive()\n\nclass AllObjectsManager(models.Manager):\n    def get_queryset(self):\n        """Менеджер для всех записей включая удалённые"""\n        return SoftDeleteQuerySet(self.model, using=self._db)\n\nclass Article(models.Model):\n    title = models.CharField(max_length=200)\n    deleted_at = models.DateTimeField(null=True, blank=True)\n\n    objects = SoftDeleteManager()      # стандартный — только живые\n    all_objects = AllObjectsManager()  # все записи\n\n# Использование:\narticle.delete()                        # мягкое удаление\nArticle.objects.all()                   # только не удалённые\nArticle.all_objects.deleted()           # только удалённые' },
        { type: 'note', value: 'При soft delete не забудь добавить индекс на deleted_at: db_index=True. И добавить unique_together или UniqueConstraint с condition=Q(deleted_at__isnull=True) чтобы уникальность работала только для живых записей.' }
      ]
    },
    {
      id: 6,
