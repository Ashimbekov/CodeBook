  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Предотвращение конфликтов миграций в команде' },
        { type: 'code', language: 'python', value: '# Пример конфликтующих миграций:\n# Разработчик А добавил поле views в Article (0003_a_views)\n# Разработчик Б добавил поле rating в Article (0003_b_rating)\n#\n# git pull -> теперь оба файла в папке migrations/\n\n# Шаги для разрешения:\n# 1. python manage.py makemigrations --merge\n# 2. Проверить созданный merge файл (0004_merge.py)\n# 3. python manage.py migrate\n\n# Содержимое merge migration:\n# class Migration(migrations.Migration):\n#     dependencies = [\n#         ("blog", "0003_a_views"),    # обе ветки\n#         ("blog", "0003_b_rating"),\n#     ]\n#     operations = []  # ничего делать не нужно!\n\n# Практики для команды:\n# - Всегда делай git pull перед makemigrations\n# - Используй атомарные PR с миграциями\n# - Не коммить --squash на продакшн без координации с командой' },
        { type: 'tip', value: 'После команды squashmigrations обязательно проверь что новый squashed файл работает на чистой БД. Запусти python manage.py migrate --run-syncdb на тестовой среде.' }
      ]
    },
    {
      id: 6,
