  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'Обслуживание медиафайлов в development' },
        { type: 'code', language: 'python', value: '# urls.py — добавить для development\nfrom django.conf import settings\nfrom django.conf.urls.static import static\n\nurlpatterns = [\n    # ... твои маршруты ...\n]\n\n# Только в режиме разработки (DEBUG=True)\nif settings.DEBUG:\n    urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)\n\n# В продакшне медиафайлы раздаёт Nginx, не Django!\n\n# Работа с файлами в view:\nfrom django.core.files.storage import default_storage\n\ndef upload_product_image(request):\n    if request.method == "POST" and request.FILES.get("image"):\n        product = get_object_or_404(Product, pk=request.POST["id"])\n        old_image = product.image  # сохранить для удаления\n        product.image = request.FILES["image"]\n        product.save()\n        # Удалить старое изображение:\n        if old_image:\n            default_storage.delete(old_image.name)\n        return redirect("product-detail", pk=product.pk)' },
        { type: 'tip', value: 'Никогда не храни пользовательские файлы в директории статики (STATIC_ROOT). Медиафайлы (загруженные пользователями) идут в MEDIA_ROOT, статика (CSS, JS, иконки) — в STATIC_ROOT.' }
      ]
    },
    {
      id: 5,
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'update_or_create и bulk_update' },
        { type: 'code', language: 'python', value: '# update_or_create — обновить если существует, создать если нет\nproduct, created = Product.objects.update_or_create(\n    sku="LAPTOP-001",                              # условие поиска\n    defaults={"price": 45000, "stock": 10}         # поля для обновления/создания\n)\n# Если запись с sku="LAPTOP-001" есть — обновит price и stock\n# Если нет — создаст с этими полями\n\n# bulk_update — массовое обновление уже загруженных объектов\nproducts = list(Product.objects.filter(is_active=True))\nfor p in products:\n    p.price = p.price * 1.1  # повышение цены на 10%\nProduct.objects.bulk_update(products, ["price"])  # один SQL UPDATE\n# Намного эффективнее чем .save() в цикле!\n\n# Транзакции при массовых операциях:\nfrom django.db import transaction\n\nwith transaction.atomic():\n    Product.objects.bulk_create([\n        Product(name=f"Товар {i}", price=i*100)\n        for i in range(1000)\n    ], batch_size=100)  # вставлять по 100 за раз' },
        { type: 'note', value: 'bulk_create не вызывает метод save() и сигналы (pre_save, post_save) каждого объекта. Если у тебя есть логика в save() или сигналах — bulk_create её пропустит. Для таких случаев используй обычный цикл с save() внутри transaction.atomic().' }
      ]
    },
    {
      id: 7,
