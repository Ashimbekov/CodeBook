  ...
  lessons: [
    ...
    },
    {
      ...
      content: [
        ...
        { type: 'heading', value: 'HTTPS с Let\'s Encrypt и оптимизация Nginx' },
        { type: 'code', language: 'python', value: '# Полный nginx.conf с HTTPS (Let\'s Encrypt через certbot):\nserver {\n    listen 80;\n    server_name example.com www.example.com;\n    # Перенаправляем HTTP -> HTTPS\n    return 301 https://$server_name$request_uri;\n}\n\nserver {\n    listen 443 ssl http2;\n    server_name example.com www.example.com;\n\n    # SSL сертификаты от Let\'s Encrypt\n    ssl_certificate /etc/letsencrypt/live/example.com/fullchain.pem;\n    ssl_certificate_key /etc/letsencrypt/live/example.com/privkey.pem;\n    ssl_protocols TLSv1.2 TLSv1.3;\n\n    # Статика с кешированием\n    location /static/ {\n        alias /app/staticfiles/;\n        expires 1y;                    # кешировать 1 год\n        add_header Cache-Control "public, immutable";\n        gzip_static on;                # отдавать .gz файлы если есть\n    }\n\n    # Ограничение размера загрузки\n    client_max_body_size 10M;\n\n    location / {\n        proxy_pass http://django;\n        proxy_set_header Host $host;\n        proxy_set_header X-Real-IP $remote_addr;\n        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;\n        proxy_set_header X-Forwarded-Proto https;\n        proxy_read_timeout 90;\n    }\n}' },
        { type: 'tip', value: 'В Django при Nginx с HTTPS добавь в settings.py: SECURE_PROXY_SSL_HEADER = ("HTTP_X_FORWARDED_PROTO", "https"). Это позволяет Django знать что запрос пришёл по HTTPS несмотря на то что Gunicorn получает HTTP.' }
      ]
    },
    {
      id: 5,
